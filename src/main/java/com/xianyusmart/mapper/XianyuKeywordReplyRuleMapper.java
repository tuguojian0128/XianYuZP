...
public interface XianyuKeywordReplyRuleMapper extends BaseMapper<XianyuKeywordReplyRule> {
    ...
    @Select("SELECT * FROM xianyu_keyword_reply_rule WHERE xianyu_account_id = #{accountId} AND xy_goods_id = #{xyGoodsId} AND match_mode = 1 AND #{message} LIKE CONCAT('%', keyword, '%') AND is_fallback = 0")
